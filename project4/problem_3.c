...
volatile uint32_t checksums[1024];
...
const char * readFile(char *dir_name, char* file_name, unsigned long size)
{
	...
	int c;
	int i=0;
	char *name_1 = malloc((strlen(dir_name)+strlen(file_name)+2)*sizeof(char));
...
void *thread_function (void *arg) {
	...
	do {
		...
		char *temp1 = malloc(sizeof(char)*(strlen(t->dirname)+strlen(t->filename)+2));
...
int main(int argc,char* argv[]){
	...
	if(dir == NULL) {
		...
		printf("Opening the directory failed, exiting");
		return EXIT_FAILURE;
	}
