...
const char * readFile(const char *dir_name, char* file_name, unsigned long size)
{
...
int main(int argc,char* argv[]){
	...
	if(dir == NULL) {
		...
		printf("Opening directory failed, exiting.. \n");
		return EXIT_FAILURE;
	}
	while(1) {
		...
		if(d->d_type == DT_REG){
			...
			char *temp1 = malloc(strlen(dirname)+strlen(d->d_name)+2);
			...
			free(temp1);
			if(size == 0) {
                                printf("File empty, skipping it\n");
				
                                checksums[i] = 0;
				i++;
                                continue;
                        }
			const char *buffer = readFile(dirname,d->d_name,size);
//			printf("%d\n",size);

			if(strcmp((const char*)buffer,"ACCESS ERROR")==0){

				printf("File : %s error: %s\n\n",d->d_name,buffer);
			}
			else {
				checksum = crc32(0, buffer, size-1);
	...
	for(j=0 ; j<i ; j++){
		...
		strcpy(temp,files[j]);
		checksum = checksums[j];
		strcpy(files[j],value);
		checksums[j] = checksums[index];
